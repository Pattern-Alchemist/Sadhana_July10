...
async function postSearch(query: string) {
  const response = await fetch("/api/archivist", {
    method: "POST",
...
export default function Archivist() {
  ...
  async function ask(q: string) {
    ...
    try {
      const { payload } = await postSearch(trimmedQuery);
      ...
      <div className="rounded-sm border border-[var(--hairline)] bg-[var(--color-ink)]/60 p-5">
        ...
        <div className="mt-3">
          <span className="text-xs italic text-[var(--color-bone)]/50">
            Weighted catalogue search across siddhis and codices (free, always available).
          </span>
        </div>
